...
let ReportsService = class ReportsService {
    ...
    async sales(query) {
        const { startDate, endDate } = this.resolveDateRange(query.period ?? 'daily', query.startDate, query.endDate);
        const where = {
    ...
    async topProducts(query) {
        const { startDate, endDate } = this.resolveDateRange(query.period ?? 'monthly', query.startDate, query.endDate);
    ...
    async revenue(query) {
      ...
      GROUP BY period
      ORDER BY period`, startDate, endDate);
    ...
    resolveDateRange(period, startStr, endStr) {
        const now = new Date();
        let startDate;
